...
- **完了画面**に遷移し、**受付番号 `FND-XXXXX`** が大きく表示される（メモしておく）
...
- 「郵送する」→ **匿名お届け先**が表示される:
  ...
  受取人: カエクル 太郎 様
  ```
- 「送信する」→ **完了画面**（受付番号 + 「郵送の方へ」の同梱メモ案内 + 「受け取り先を登録する」導線）
...
2. 「**「黒い長財布」の落とし物が届け出られました**」＋ 届け方（手渡し（渋谷駅前交番）/ 郵送（渋谷郵便局 留め））・メモ・受付番号・日時 が表示される
   - マイページの持ち物カードにも「未読 1」バッジが付く
